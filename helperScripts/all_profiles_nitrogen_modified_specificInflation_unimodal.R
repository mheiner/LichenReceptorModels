...
# inflation_factor <- 1
A_factor <- 1
# min_alpha = 2.0 # supplied in calling script
...
agriculture_sds = pmin(agriculture_sds, agriculture_means / sqrt(min_alpha))
...
c_c_sds = pmin(c_c_sds, c_c_means / sqrt(min_alpha))
...
unpaved_sds = pmin(unpaved_sds, unpaved_means / sqrt(min_alpha))
...
coal_plant_sds = pmin(coal_plant_sds, coal_plant_means / sqrt(min_alpha))
...
oil_sds = pmin(oil_sds, oil_means / sqrt(min_alpha))
...
playa_sds = pmin(playa_sds, playa_means / sqrt(min_alpha))
...
copper_sds = pmin(copper_sds, copper_means / sqrt(min_alpha))
...
highway_sds = pmin(highway_sds, highway_means / sqrt(min_alpha))
...
rock_sds = pmin(rock_sds, rock_means / sqrt(min_alpha))
...
cement_sds = pmin(cement_sds, cement_means / sqrt(min_alpha))
...
brake_sds = pmin(brake_sds, brake_means / sqrt(min_alpha))
...
exhaust_sds = pmin(exhaust_sds, exhaust_means / sqrt(min_alpha))
...
baseline_rhizoplaca_sds = pmin(baseline_rhizoplaca_sds, baseline_rhizoplaca_means / sqrt(min_alpha))
...
empty_alpha = rep(min_alpha, ncol(alpha_matrix))
...
natural_beta <- empty_alpha / natural_weights
anthropogenic_beta <- empty_alpha / anthropogenic_weights
# empty_beta <- rep(1, ncol(alpha_matrix))

empty_alpha / natural_beta
sqrt(empty_alpha) / natural_beta

sqrt(empty_alpha) / empty_alpha